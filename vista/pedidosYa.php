...
$consultaPedidoPromocion=" SELECT pedidosya.id PEDIDOSYAID,pedidosya.NUMEROPROMOCION,pedidopromocion.ID,pedidopromocion.TOTAL FROM pedidosya
                           inner join pedidopromocion on pedidosya.PEDIDOPROMOCIONID = pedidopromocion.id
                           where pedidosya.MOSTRAR = true
                           order by NUMEROPROMOCION asc";
$resConsultaPedidoPromocion = $conexion->query($consultaPedidoPromocion); 

////////////////////////////////////////////////////////////////////////////////////////////

$listPedidos = "SELECT btn_pedidosya_tmp.BOTONID,pedidosya.NUMEROPROMOCION,btn_pedidosya_tmp.PRECIO,PEDIDOPROMOCION.TOTAL PRECIOFIJO, btn_pedidosya_tmp.CANTIDAD FROM btn_pedidosya_tmp
...
if (isset($_GET["accion"])) {
    if (($_GET["accion"]) == 'agregarPedido') {
        if (isset($_GET["id"])) {
            $idBoton=$_GET["id"];
            $precio=$_GET["precio"];
            $agregarPedidosYa = "select * from BTN_PEDIDOSYA_TMP where botonid=$idBoton" ;
            ...
            if ($resultAgregarPedidosYa > 0) {
                $borrarDatosTablaTemporal = "delete from BTN_PEDIDOSYA_TMP where BOTONID = $idBoton";
                $conexion->query($borrarDatosTablaTemporal);
            } else {
                $guardarDatosTablaTemporal = "insert into BTN_PEDIDOSYA_TMP (BOTONID,CANTIDAD,PRECIO) values ($idBoton,1,$precio)";
        ...
        $actualizarCantidadPrecio = "update BTN_PEDIDOSYA_TMP set CANTIDAD=$cantidad, precio=$precio where botonid=$id";
...
<html>
    <div align="center"><h1>Pedidos ya</h1></div>
         ...
         <?php
             while ($registroConsultaPedidoPromocion = $resConsultaPedidoPromocion->fetch_array(MYSQLI_BOTH)) {
                 ...
                 echo '<tr align="center" >
                        ...
                        $btnPedidosYa="SELECT * FROM BTN_PEDIDOSYA_TMP";
