<?php
 include ("../conexion/conexion.php");
    ...
    ////////////////////////////////////////////////////////////////////////777
    $listDelicias = "SELECT btn_delicia_tmp.BOTONID,delicia.NOMBRE,btn_delicia_tmp.PRECIO,delicia.PRECIO PRECIOFIJO,btn_delicia_tmp.CANTIDAD CANTIDADBTN,delicia.CANTIDAD CANTIDADDEL FROM btn_delicia_tmp
    ...
    if (isset($_GET["accion"])) {
        if (($_GET["accion"]) == 'guardarDelicia') {
            if (isset($_GET["id"])) {
                $idBoton = $_GET["id"];
                $cantBtnDelicia = "SELECT * FROM BTN_DELICIA_TMP where BOTONID = $idBoton";
                $resCantBtnDelicia = $conexion->query($cantBtnDelicia);
                $resultCantBtnDelicia = $resCantBtnDelicia->num_rows;

                if ($resultCantBtnDelicia > 0) {
                    $borrarDatosTablaTemporal = "delete from BTN_DELICIA_TMP where BOTONID = $idBoton";
                    ...
                    $guardarDatosTablaTemporal = "insert into BTN_DELICIA_TMP (BOTONID,CANTIDAD,PRECIO) value ($idBoton,$cantidad,$precio)";
                    $conexion->query($guardarDatosTablaTemporal);
                }
                Header("Location: delicias.php");
            }
            ...
            if (isset($_GET["id"])) {
                ...
                $actualizarCantidadPrecio = "update btn_delicia_tmp set CANTIDAD=$cantidad, precio=$precio where botonid=$id";
...
<script type="text/javascript">
    function guardarDelicia(id){
        window.location.href = "delicias.php?accion=guardarDelicia&id=" + id;
    }
    ...
    function guardarCantidadPrecioDeliciaCantidadFija(valor){
        ...
        window.location.href = "delicias.php?accion=guardarCantidadPrecio&id=" + id+"&cantidad="+cantidad+"&precio="+precio;
    }

    function borrarSeleccion(){
        window.location.href = "delicias.php?accion=borrarSeleccion";        
    }
</script>
                    ...
                    <table align="center">
                          <?php
                            ...
                            while ($registroDelicia = $resDelicia->fetch_array(MYSQLI_BOTH)) {
                                ...
                                $btnDelicias="SELECT * FROM btn_delicia_tmp";
