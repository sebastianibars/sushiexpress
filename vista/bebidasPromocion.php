<?php
 include ("../conexion/conexion.php");
    ...
    ////////////////////////////////////////////////////////////////////////777
    $listBebidas = "SELECT btn_bebida_prom_tmp.BOTONID,bebida.NOMBRE,btn_bebida_prom_tmp.PRECIO PRECIOBTN,bebida.PRECIO PRECIOFIJO,btn_bebida_prom_tmp.CANTIDAD FROM btn_bebida_prom_tmp
    ...
    if (isset($_GET["accion"])) {
        if (($_GET["accion"]) == 'guardarBebida') {
            if (isset($_GET["id"])) {
                $idBoton = $_GET["id"];
                $cantBtnBebida = "SELECT * FROM btn_bebida_prom_tmp where BOTONID = $idBoton";
                $resCantBtnBebida = $conexion->query($cantBtnBebida);
                $resultCantBtnBebida = $resCantBtnBebida->num_rows;

                if ($resultCantBtnBebida > 0) {
                    $borrarDatosTablaTemporal = "delete from btn_bebida_prom_tmp where BOTONID = $idBoton";
                    ...
                    $guardarDatosTablaTemporal = "insert into btn_bebida_prom_tmp (BOTONID,CANTIDAD,PRECIO) value ($idBoton,$cantidad,$precio)";
                    $conexion->query($guardarDatosTablaTemporal);
                }
                Header("Location: bebidasPromocion.php");
            }
            ...
            $actualizarCantidadPrecio = "update btn_bebida_prom_tmp set CANTIDAD=$cantidad, precio=$precio where botonid=$id";
            $conexion->query($actualizarCantidadPrecio);
               
            Header("Location: bebidasPromocion.php");
        }else if (($_GET["accion"]) == 'borrarSeleccion') {
            $borrarTablas="DELETE FROM btn_bebida_prom_tmp";
...
<script type="text/javascript">
    function guardarBebida(id){
        window.location.href = "bebidasPromocion.php?accion=guardarBebida&id=" + id;
    }
    ...
    function guardarCantidadPrecioBebidaCantidadFija(valor){
        ...
        window.location.href = "bebidasPromocion.php?accion=guardarCantidadPrecio&id=" + id+"&cantidad="+cantidad+"&precio="+precio;
    }

    function borrarSeleccion(){
        window.location.href = "bebidasPromocion.php?accion=borrarSeleccion";
    }
</script>
                    ...
                    <table align="center">
                          <?php
                            ...
                            while ($registroBebida = $resBebidas->fetch_array(MYSQLI_BOTH)) {
                                ...
                                $btnBebidas="SELECT * FROM btn_bebida_prom_tmp";
